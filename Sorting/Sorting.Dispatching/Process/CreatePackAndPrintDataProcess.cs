    ...
    class CreatePackAndPrintDataProcess : AbstractProcess
    {
        ...
        private void CreatePackAndPrintDataFile(int exportNo)
        {
            try
            {
                ...
                string packDataFileName = System.DateTime.Now.ToString("yyyy-MM-dd") + " [" + (exportNo == 0 ? "All" : (exportNo == 1 ? "ONE" : "TWO")) + "].ORDER";
                string s = "";
            ...
            {
                Logger.Error("CreatePackAndPrintDataFile 方法出错" + e.Message);
            }
        }        
    }
}
