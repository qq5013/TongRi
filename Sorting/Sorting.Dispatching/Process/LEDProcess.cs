...
namespace Sorting.Dispatching.Process
{
    public class LEDProcess : AbstractProcess
    {
            ...
            try
            {
                ...
                switch (stateItem.ItemName)
                {
                    ...
                    case "Empty32":
                        ...
                        if (errAddress3 == 0)
                        {
                            foreach (KeyValuePair<int, int> kv in emptyAddress3)
                            {
                                BreakShow("0", "3", kv.Value);
                            }
                            
                            emptyAddress3 = new Dictionary<int, int>();
                        }
                    ...
                    case "Empty2":
                        ...
                        if (errAddress2 == 0)
                        {
                            foreach (KeyValuePair<int, int> kv in emptyAddress2)
                            {
                                BreakShow("0", "2", kv.Value);
                            }
                            emptyAddress2 = new Dictionary<int, int>();
                        }
                        else if (errAddress2 > 0)
                        {
                            if (!emptyAddress2.ContainsKey(errAddress2))
                                emptyAddress2.Add(errAddress2, errAddress2);

                            Logger.Info("立式机机地址" + errAddress2.ToString() + "显示缺烟");
                            BreakShow("1", "2", errAddress2);
                        }
                        break;
                    case "BreakDown31":
                        //通道机卡烟报警
                        ...
                        if (bdAddress3 == 0)
                        {
                            foreach (KeyValuePair<int, int> kv in breakAddress3)
                            {
                                BreakShow("0", "3", kv.Value);
                            }

                            breakAddress3 = new Dictionary<int, int>();
                        }
                        else if (bdAddress3 > 0)
                        {
                            if (!breakAddress3.ContainsKey(bdAddress3))
                                breakAddress3.Add(bdAddress3, bdAddress3);

                            BreakShow("2", "3", bdAddress3);
                        }
                        break;
                    case "BreakDown2":
                        //立式机卡烟报警
                        ...
                        if (bdAddress2 == 0)
                        {
                            foreach (KeyValuePair<int, int> kv in breakAddress2)
                            {
                                BreakShow("0", "2", kv.Value);
                            }

                            breakAddress2 = new Dictionary<int, int>();
                        }
                        else if (bdAddress2 > 0)
                        {
                            if (!breakAddress2.ContainsKey(bdAddress2))
                                breakAddress2.Add(bdAddress2, bdAddress2);

                            BreakShow("2", "2", bdAddress2);
                        }
                        break;
        ...
        private void WriteRestartDataToPLC()
        {
            try
            {
                WriteToService("SortPLC", "LastOrder2", 0);
                WriteToService("SortPLC", "LastOrder3", 0);
