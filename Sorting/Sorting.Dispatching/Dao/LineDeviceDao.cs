...
namespace Sorting.Dispatching.Dao
{
    public class LineDeviceDao : BaseDao
    {
        ...
        {
            string sql = "SELECT * FROM CMD_LINEDEVICE WHERE LINECODE = '{0}' ORDER BY PRIORITY";
            ...
            string sql = @" SELECT a.linecode,a.batchno,CONVERT(varchar(10),a.orderdate,120) AS orderdate
                            ...
                            LEFT JOIN 
                                (SELECT distinct a.orderdate,b.batchno,b.routecode, a.cigarettecode 
                                    FROM SC_I_ORDERdetail a,SC_I_ORDERmaster b WHERE a.orderid=b.orderid AND a.cigarettecode 
                                    NOT IN (SELECT CIGARETTECODE FROM CMD_Product WHERE ISABNORMITY='1')
                                ) b
                            ...
                            LEFT JOIN 
                                (SELECT E.linecode,E.channelcount,ISNULL
                                    ...
                                    FROM (SELECT * FROM CMD_LINEDEVICE WHERE channeltype='2'
                                    )E
                                LEFT JOIN 
                                    (SELECT * FROM CMD_LINEDEVICE WHERE channeltype='3')F
        ...
        {
            string strSQL = "update CMD_LINEDEVICE set OCCUPYFOURTOWER='"+OccupyFourCh+"',OCCUPYTHREETOWER='"+OccupyThreeCh+"',OCCUPYTWOTOWER='"+OccupyTwoCh+"' WHERE LINECODE='"+linecode+"' AND CHANNELTYPE='2'";
        ...
        public string GetLineCHPara(string linecode,string OccupyCount)
        {
            ...
            { 
                case "4":
                    strSQL = "SELECT isnull(OCCUPYFOURTOWER,0) FROM CMD_LINEDEVICE WHERE LINECODE='" + linecode + "' AND CHANNELTYPE='2'";
                    break;
                case "3":
                    strSQL = "SELECT isnull(OCCUPYTHREETOWER,0) FROM CMD_LINEDEVICE WHERE LINECODE='" + linecode + "' AND CHANNELTYPE='2'";
                    break;
                case "2":
                    strSQL = "SELECT isnull(OCCUPYTWOTOWER,0) FROM CMD_LINEDEVICE WHERE LINECODE='" + linecode + "' AND CHANNELTYPE='2'";
                    break;
                default:
                    return "0";                    
            }
